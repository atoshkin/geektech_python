...
lists = ['full_name.txt', 'email.txt', 'file_name.txt', 'color.txt']
    ...
    with open('MOCK_DATA.txt') as fread:
        for line in fread:
            ...
            if len(s) > 4:
                s[0] += ' ' + s.pop(1)
            info = Data(*s)
            with open(lists[0], 'a') as f0:
                f0.write(info.full_name)
                f0.write('\r')
            with open(lists[1], 'a') as f1:
                f1.write(info.email)
                f1.write('\r')
            with open(lists[2], 'a') as f2:
                f2.write(info.file_name)
                f2.write('\r')
            with open(lists[3], 'a') as f3:
                f3.write(info.color)
                f3.write('\r')
